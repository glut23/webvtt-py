...
class SBVParserTestCase(GenericParserTestCase):
    ...
    def test_sbv_total_length(self):
        self.assertEqual(
            self.webvtt.from_sbv(self._get_file('sample.sbv')).total_length,
            16
        )

    def test_sbv_parse_captions(self):
        self.assertEqual(
            len(self.webvtt.from_srt(self._get_file('sample.srt')).captions),
            5
        )
    ...
    def test_sbv_timestamps_format(self):
        self.webvtt.from_sbv(self._get_file('sample.sbv'))
        self.assertEqual(self.webvtt.captions[1].start, '00:00:11.378')
        self.assertEqual(self.webvtt.captions[1].end, '00:00:12.305')

    def test_sbv_timestamps_in_seconds(self):
        self.webvtt.from_sbv(self._get_file('sample.sbv'))
        self.assertEqual(self.webvtt.captions[1].start_in_seconds, 11.378)
        self.assertEqual(self.webvtt.captions[1].end_in_seconds, 12.305)

    def test_sbv_get_caption_text(self):
        self.webvtt.from_sbv(self._get_file('sample.sbv'))
        self.assertEqual(self.webvtt.captions[1].text, 'Caption text #2')
